...
async function refreshData() {
    try {
        ...
        // data.humidity e data.temperature podem ser null enquanto o sensor ainda não leu
        if (hum)  hum.textContent  = (data.humidity    != null ? data.humidity    : '--') + '%';
        if (temp) temp.textContent = (data.temperature != null ? data.temperature : '--') + '°C';
    } catch (e) {
...
async function populateWifiList() {
    ...
    try {
        ...
        const data = await response.json();
        // O servidor retorna { networks: [{ssid, rssi}, ...] }
        const networks = data.networks || [];
        list.innerHTML = '';
        if (networks.length === 0) {
            list.innerHTML = '<p style="color:var(--text-s); text-align:center;">Nenhuma rede encontrada</p>';
            return;
        }
        ...
        list.innerHTML = '<p style="color:var(--text-s);">Erro ao carregar redes</p>';
    }
}
