...
# Resolution: 240 x 240 pixels (round) | Interface: SPI
# EXACT COPY from CODE_PROJECT (working version)
...
class GC9A01:
    ...
    def _data(self, data):
        ...
        if isinstance(data, int): self.spi.write(bytes([data]))
    ...
    def fill(self, color):
        ...
        line = ustruct.pack(">H", color) * 240
        self.dc.value(1); self.cs.value(0)
        for _ in range(240): self.spi.write(line)
        self.cs.value(1)

    def fill_rect(self, x, y, w, h, color):
        x1 = min(x + w - 1, 239); y1 = min(y + h - 1, 239)
        self._set_window(x, y, x1, y1)
        cb = ustruct.pack(">H", color)
        count = (x1 - x + 1) * (y1 - y + 1)
        self.dc.value(1); self.cs.value(0)
        for _ in range(count): self.spi.write(cb)
        self.cs.value(1)
