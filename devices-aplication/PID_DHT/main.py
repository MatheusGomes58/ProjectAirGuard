        ...
        try:
            ...
            if mode == "auto":
                ...
                for act in actions:
                    ...
                    sp_val = sp["value"]
                    condition = sp.get("condition", "above")
                    period = act["period"]
                    relay = act["relay"]
                    act_id = act["id"]
            ...
            if temp is not None and hum is not None:
                if mode == "auto":
                    shared_state.add_history_point(temp, hum, relay_duty[1], relay_duty[2], relay_duty[3], rpm)
                else:
                    shared_state.add_history_point(temp, hum, 1.0 if r1_on else 0.0, 1.0 if r2_on else 0.0, float(r3_on), rpm)
        except Exception as e:
            log("[control] Erro no loop de controle: {}".format(e))
            try:
                import sys
